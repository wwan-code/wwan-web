...
const extractAndMergeUserData = (apiResponseData, existingUserState = null) => {
    ...
    // Loại bỏ các trường undefined để không ghi đè state cũ với undefined
    Object.keys(extracted).forEach(key => extracted[key] === undefined && delete extracted[key]);

    if (existingUserState) {
        // Merge dữ liệu mới vào state user hiện tại
        return { ...existingUserState, ...extracted };
    }
    // Nếu không có existingUserState (ví dụ: login lần đầu), trả về dữ liệu đã trích xuất
    // nhưng cần đảm bảo các trường cơ bản có giá trị mặc định nếu API không trả về
    return {
        ...initialState.user, // Lấy cấu trúc từ initialState để đảm bảo các trường không bị thiếu
        ...extracted,
...
export const loginUser = createAsyncThunk("user/loginUser", async (userData, { rejectWithValue }) => {
    try {
        ...
        // API login nên trả về đầy đủ thông tin user, bao gồm cả privacySettings
        const user = extractAndMergeUserData(response.data); // Sử dụng hàm mới
    ...
    reducers: {
        ...
        setActiveItem: (state, action) => {
            if (state.user && action.payload) {
                ...
                state.user = { ...state.user }; // Ensure new object for re-render
                switch (itemType) {
