...
import Pagination from '@components/Common/Pagination'; // Tạo component này nếu chưa có
import UserAvatarDisplay from '@components/Common/UserAvatarDisplay'; // Component hiển thị avatar
                ...
                >
                    {!coverImage && (
                        <div className="collection-card-public__cover-placeholder">
                            <i className="fas fa-photo-video fa-2x"></i> {/* Icon placeholder */}
                        </div>
                    )}
                    ...
                    {previewItems && previewItems.length > 0 && (
                        <div className="collection-card-public__preview-items">
                            {previewItems.slice(0, 3).map(item => (
                                <img
                                    key={item.id}
                                    src={item.poster || item.coverImage ? (
                                        (item.poster || item.coverImage).startsWith('http') ? (item.poster || item.coverImage) : `/${(item.poster || item.coverImage)}`
...
const CollectionsPage = () => {
    ...
    const currentTypeFilter = searchParams.get('type') || ''; // 'movie', 'comic', or empty for all
    const currentSortBy = searchParams.get('sort') || 'updatedAt_desc'; // Ví dụ: updatedAt_desc, name_asc
    ...
    useEffect(() => {
        const fetchPublicCollections = async (page, q, type, sort) => {
            ...
            try {
                const sortParams = sort.split('_'); // ví dụ: "updatedAt_desc" -> ["updatedAt", "desc"]
                const params = {
                    page,
