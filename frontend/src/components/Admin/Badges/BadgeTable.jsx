...
const BadgeTable = ({ displayedData, onEdit, onDelete, requestSort, sortConfig, isDataLoading, isSubmittingAction }) => {
    ...
    return (
        <div className="table-responsive-custom admin-table-wrapper">
                        ...
                        displayedData.map((badge) => (
                            <tr key={badge.id}>
                                ...
                                <td className="text-center table-actions">
                                    <button className="btn-icon btn--sm btn--outline-primary me-1" onClick={() => onEdit(badge.id)} disabled={isSubmittingAction} title="Sửa">
                                        <i className="fas fa-edit"></i>
                                    </button>
                                    <button className="btn-icon btn--sm btn--outline-danger" onClick={() => onDelete(badge.id)} disabled={isSubmittingAction} title="Xóa">
