                                ...
                                <Link to={item.itemType === 'movie' ? `/album/${item.slug}` : `/truyen/${item.slug}`} className="result-item-link">
                                    <div className="result-item-thumbnail flex-shrink-0">
                                        <div className="result-item-img-wrapper">
                                            <img
                                                src={
                                                    (item.itemType === 'movie' && item.image) ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/${item.image}` :
                                                    (item.itemType === 'comic' && item.coverImage) ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/uploads/${item.coverImage}` :
                                                    '/img/placeholder.jpg'
                                                }
                                                alt={item.title}
