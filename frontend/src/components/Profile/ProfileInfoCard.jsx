...
import UserAvatarDisplay from '@components/Common/UserAvatarDisplay'; // Giả sử bạn có component này để hiển thị avatar
// Import SCSS nếu có style riêng cho Card này, hoặc style chung trong _profile-page.scss
// import '@assets/scss/components/profile/_profile-info-card.scss';

const ProfileInfoCard = ({
    currentUser, // User object đầy đủ của người đang đăng nhập
    profileEditData, // State form chứa dữ liệu đang chỉnh sửa
    isEditing,
    onEditToggle,
    onProfileDataChange, // Hàm xử lý thay đổi input (name, phoneNumber)
    onAvatarChange,      // Hàm xử lý thay đổi file avatar
    onRemoveAvatar,      // Hàm xử lý xóa avatar đã chọn/hiện tại
    onSocialLinkChange,  // Hàm xử lý thay đổi social links
    onSaveChanges,
    isLoading // Trạng thái loading khi submit form
}) => {
    // Hàm lấy vai trò (giữ nguyên hoặc chuyển ra utils nếu dùng ở nhiều nơi)
    const getRoleName = (roleName) => {
        switch (roleName) {
            case 'user': return 'Thành viên'; // Giả sử role trả về là 'user'
                ...
                <form onSubmit={onSaveChanges} className="custom-form profile-edit-form">
                    <div className="profile-edit-avatar-section">
                        <label htmlFor="profileAvatarFile" className="profile-edit-avatar-label">
                            <UserAvatarDisplay
                                userToDisplay={{ avatar: profileEditData.avatarPreview || currentUser?.avatar }} // Hiển thị preview hoặc avatar cũ
                                size="100"
