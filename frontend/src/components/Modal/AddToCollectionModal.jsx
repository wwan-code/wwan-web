...
const AddToCollectionModal = ({ show, onHide, itemType, itemId, itemTitle }) => {
    ...
    const fetchUserCollectionsByType = useCallback(async () => {
        ...
        setError(null);
        try {
            // API backend cần hỗ trợ param `type` để lọc
            const response = await axios.get(`/api/watchlists?type=${itemType}&includeItems=false`, { headers: authHeader() });
            if (response.data.success) {
                const fetchedCollections = response.data.watchlists || [];
                setCollections(fetchedCollections);
                if (fetchedCollections.length > 0) {
                    setSelectedCollectionId(fetchedCollections[0].id); // Chọn collection đầu tiên mặc định
                } else {
                    setSelectedCollectionId('');
                    setShowCreateNewCollectionForm(true); // Tự động hiển thị form tạo mới nếu chưa có collection nào
                }
            } else {
    ...
    useEffect(() => {
        ...
        if (!show) {
            ...
            fetchUserCollectionsByType(); // Fetch lại khi modal được mở
        }
    ...
    return (
        <div className={classNames("custom-modal-overlay", { "show": show })} onClick={onHide}>
            <div className={classNames("custom-modal custom-modal-md add-to-collection-modal", { "show": show })} onClick={e => e.stopPropagation()}>
                <div className="custom-modal-header">
                    <h5 className="custom-modal-title">Thêm "{itemTitle}" vào Bộ sưu tập</h5>
                    <button type="button" className="btn-close-custom" onClick={onHide} aria-label="Close" disabled={submitting}>&times;</button>
                </div>
