    ...
    try {
        ...
        const existingConsumableInventory = await UserInventory.findOne({
            where: {
                userId,
                shopItemId: item.id,
                // Chỉ gộp nếu là vật phẩm không có thời hạn hoặc thời hạn sắp tới giống hệt
                // Nếu vé có thời hạn khác nhau nên coi là entry riêng
        ...
        if (item.type === 'AD_SKIP_TICKET' && existingConsumableInventory) {
            // Nếu là vé và user đã có loại vé này (cùng hạn sử dụng nếu có)
            existingConsumableInventory.quantity += 1; // Mặc định mỗi lần mua là 1 vé
            // Nếu ShopItem.value dùng để chỉ số lượng vé trong 1 gói, thì += parseInt(item.value) || 1;
            await existingConsumableInventory.save({ transaction: t });
            inventoryEntry = existingConsumableInventory;
        } else {
            // Vật phẩm khác hoặc user chưa có loại vé này (hoặc vé có thời hạn khác)
            const [newEntry, created] = await UserInventory.findOrCreate({
                where: { userId, shopItemId: item.id, expiresAt: expiresAt }, // Thêm expiresAt vào where để phân biệt
                defaults: {
                    userId,
                    shopItemId: item.id,
                    expiresAt,
                    isActive: false,
                    quantity: 1 // Mặc định mỗi lần mua là 1 vé
                },
                transaction: t
            });
...
export const getUserInventory = async (req, res) => {
    ...
    try {
        const inventory = await UserInventory.findAll({
            where: {
                userId,
                // (Tùy chọn) Chỉ lấy vật phẩm chưa hết hạn hoặc vĩnh viễn
                [Op.or]: [
                    { expiresAt: null },
                    { expiresAt: { [Op.gt]: new Date() } }
                ]
            ...
            include: [{
                model: ShopItem,
                as: 'itemDetails', // Đảm bảo alias này đúng
...
// Kích hoạt/Hủy kích hoạt một vật phẩm trong kho đồ
